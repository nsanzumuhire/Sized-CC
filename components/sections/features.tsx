...
export function Features() {
  return (
    <section className="py-32 bg-neutral-100 dark:bg-neutral-950 border-t border-black/5 dark:border-white/5">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 className="text-4xl md:text-5xl font-bold font-heading text-black dark:text-white mb-6 tracking-tight">
            ...
            <div className="space-y-4">
              {features.map((feature, index) => (
                <motion.div 
                  key={index} 
                  className="flex gap-4 p-5 rounded-xl border border-black/5 dark:border-white/5 hover:border-black/10 dark:hover:border-white/10 bg-black/[0.02] dark:bg-white/[0.02] hover:bg-black/[0.04] dark:hover:bg-white/[0.04] transition-all duration-300"
                ...
                >
                  ...
                  <div>
                    <h4 className="font-bold text-black dark:text-white mb-1 font-heading">{feature.title}</h4>
          ...
          <div className="relative h-[450px] w-full rounded-2xl overflow-hidden">
            ...
            {/* Animated rings */}
            <motion.div 
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 border border-black/5 dark:border-white/5 rounded-full"
              animate={{ scale: [1, 1.1, 1], opacity: [0.3, 0.6, 0.3] }}
              transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
            />
            <motion.div 
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-60 h-60 border border-black/10 dark:border-white/10 rounded-full"
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-[120px] md:text-[150px] font-bold text-black/[0.03] dark:text-white/[0.03] font-heading select-none leading-none">
                100
              </span>
              <span className="text-2xl font-bold text-black/20 dark:text-white/20 font-heading -mt-8">
                % Satisfaction
              </span>
            </div>
          </div>
